...
<svg xmlns="http://www.w3.org/2000/svg" width="300" height="200">
  ...
  <text x="text-align:center" y="30" font-family="Verdana, sans-serif" font-size="22" fill="#ffffff">
    Energy: ${energy.current}/${energy.maximum}
  </text>
  ${pill(text-align:center, 50, drugReady ? "Drug CD: READY" : `Drug CD: ${formatSeconds(drugSecs)}`, drugReady)}
  ${pill(text-align:center, 100, boosterReady ? "Booster CD: EMPTY" : `Booster CD: ${formatSeconds(boosterSecs)}`, boosterReady)}
  ${pill(text-align:center, 150, refillAvailable ? `Refill: AVAILABLE (${REFILL_COST}pts)` : "Refill: USED TODAY", refillAvailable)}
